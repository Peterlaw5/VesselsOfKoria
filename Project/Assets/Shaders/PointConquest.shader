...
Shader "Vok/PointConquestCircle"
{
	Properties
	{
	...
	#include "UnityCG.cginc"
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float stepFunc = step(cutout / (0.0001 + step(0.0001,_Level) * (_Level - 0.0001)), _Level);
				col = tex2D(_MainTex, i.uv) * (c + stepFunc * (emissive - c));
				col += stepFunc * emissive * pow(_EmissiveAmount, 2.2);
				/*
				if (_Level == 0.0)
				{
					//float4 emissive = i.color * col.a*_Color;
